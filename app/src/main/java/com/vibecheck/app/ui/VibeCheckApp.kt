...
import com.vibecheck.app.audio.AmbientMusicController
...
@Composable fun VibeCheckApp(incomingChallenge:Challenge?=null,onChallengeConsumed:()->Unit={},gameViewModel:GameViewModel=viewModel(),soloPartyViewModel:SoloPartyViewModel=viewModel()){
 val context=LocalContext.current;val lifecycleOwner=LocalLifecycleOwner.current;val animatorScale=remember{runCatching{Settings.Global.getFloat(context.contentResolver,Settings.Global.ANIMATOR_DURATION_SCALE,1f)}.getOrDefault(1f).coerceAtLeast(0f)};val billing=remember{PremiumBillingManager(context.applicationContext)};val groups=remember{PlayerGroupStore(context.applicationContext)};val stats=remember{LocalStatsStore(context.applicationContext)};val onboarding=remember{OnboardingStore(context.applicationContext)};val appearance=remember{AppearanceStore(context.applicationContext)};val audio=remember{AudioPreferencesStore(context.applicationContext)};val musicController=remember{AmbientMusicController(context.applicationContext,R.raw.vibecheck_pastel_ambient)};val theme by appearance.selectedStyle.collectAsState(initial=VibeThemeStyle.PASTEL_DRAWN);val musicEnabled by audio.musicEnabled.collectAsState();val musicVolume by audio.musicVolume.collectAsState();val scope=rememberCoroutineScope();var showOnboarding by remember{mutableStateOf(!onboarding.isCompleted())};var settingsOpen by rememberSaveable{mutableStateOf(false)};var soloOpen by rememberSaveable{mutableStateOf(false)}
 ...
 VibeCheckTheme(theme){Surface(Modifier.fillMaxSize(),color=Color.Transparent){VibeBackdrop(theme){Box(Modifier.fillMaxSize().safeDrawingPadding().padding(20.dp)){
  when{soloOpen&&screen==AppScreen.HOME->SoloPartyScreen(visible,selectedIds,soloSetupMode,query,setupName,validation,canStart,{soloOpen=false},soloPartyViewModel::setQuery,soloPartyViewModel::setPlayerName,soloPartyViewModel::setMode,soloPartyViewModel::toggle,{soloPartyViewModel.autoCompose(System.nanoTime(),5)},{if(canStart){gameViewModel.startSoloSession(selectedIds,soloSetupMode,System.currentTimeMillis(),setupName);soloOpen=false}})
