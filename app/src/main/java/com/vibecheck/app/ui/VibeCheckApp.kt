...
@Composable fun VibeCheckApp(incomingChallenge:Challenge?=null,onChallengeConsumed:()->Unit={},gameViewModel:GameViewModel=viewModel(),soloPartyViewModel:SoloPartyViewModel=viewModel()){
 val context=LocalContext.current;val animatorScale=remember{runCatching{Settings.Global.getFloat(context.contentResolver,Settings.Global.ANIMATOR_DURATION_SCALE,1f)}.getOrDefault(1f).coerceAtLeast(0f)};val billing=remember{PremiumBillingManager(context.applicationContext)};val history=remember{QuestionHistoryStore(context.applicationContext)};val groups=remember{PlayerGroupStore(context.applicationContext)};val stats=remember{LocalStatsStore(context.applicationContext)};val onboarding=remember{OnboardingStore(context.applicationContext)};val appearance=remember{AppearanceStore(context.applicationContext)};val theme by appearance.selectedStyle.collectAsState(initial=VibeThemeStyle.PASTEL_DRAWN);val scope=rememberCoroutineScope();var showOnboarding by remember{mutableStateOf(!onboarding.isCompleted())};var settingsOpen by rememberSaveable{mutableStateOf(false)};var soloOpen by rememberSaveable{mutableStateOf(false)}
 ...
 VibeCheckTheme(theme){Surface(Modifier.fillMaxSize(),color=Color.Transparent){VibeBackdrop(theme){Box(Modifier.fillMaxSize().safeDrawingPadding().padding(20.dp)){
  when{soloOpen&&screen==AppScreen.HOME->SoloPartyScreen(visible,selectedIds,kind,query,setupName,validation,canStart,{soloOpen=false},soloPartyViewModel::setQuery,soloPartyViewModel::setPlayerName,soloPartyViewModel::setKindFilter,soloPartyViewModel::toggle,{soloPartyViewModel.autoCompose(System.nanoTime(),5)},{if(canStart){gameViewModel.startSoloSession(selectedIds,GameMode.WHO_OF_US,System.currentTimeMillis(),setupName);soloOpen=false}})
   settingsOpen&&screen==AppScreen.HOME->SettingsScreen(theme,{scope.launch{appearance.save(it)}},{settingsOpen=false})
   showOnboarding&&incomingChallenge==null&&screen==AppScreen.HOME->OnboardingScreen{onboarding.markCompleted();showOnboarding=false}
    ...
    AppScreen.GAME->{val questions=QuestionRepository.forMode(mode,seed,emptySet(),sessionIntensity,sessionPack);if(questions.isEmpty())Text("Questions indisponibles") else{val i=qi.coerceIn(0,questions.lastIndex);val q=questions[i];if(solo){val options=if(mode==GameMode.RED_GREEN)listOf("Green Flag","Red Flag") else listOf(soloName)+personas.map{it.displayName};if(soloAnswer.isBlank())GameScreen(mode,"$soloName, à toi de répondre.\n\n${q.text}",i+1,questions.size,options,gameViewModel::submitSoloPlayerAnswer,gameViewModel::abandonGame,animatorScale) else{val simulated=personas.map{PersonaAnswerEngine.answer(it,q.id,q.text,options,seed)};GameScreen(mode,"Les personnages répondent après toi.\n\n${q.text}",i+1,questions.size,emptyList(),{},gameViewModel::abandonGame,animatorScale,simulated.mapIndexed{idx,a->SimulatedResponseUi(personas[idx].displayName,a.option,a.isFictionalSimulation)}){gameViewModel.submitSoloRound(q.id,simulated,i==questions.lastIndex)}}}else GameScreen(mode,q.text,i+1,questions.size,if(mode==GameMode.RED_GREEN)listOf("Green Flag","Red Flag") else players,{gameViewModel.answer(q.id,it,i==questions.lastIndex)},gameViewModel::abandonGame,animatorScale)}}
