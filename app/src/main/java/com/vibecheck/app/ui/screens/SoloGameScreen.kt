...
@Composable
fun SoloGameScreen(questionText: String, progress: Int, total: Int, cast: List<Persona>, round: SoloRoundResult, onNext: () -> Unit, onExit: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
            ...
            Text("Quitter", modifier = Modifier.clickable(onClick = onExit).padding(8.dp), color = MaterialTheme.colorScheme.onSurfaceVariant, style = MaterialTheme.typography.labelLarge)
        }
        ...
        if (!revealed) {
            Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(22.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.primary.copy(alpha = .10f)), border = BorderStroke(1.dp, MaterialTheme.colorScheme.primary.copy(alpha = .24f))) {
        ...
        AnimatedVisibility(visible = revealed) {
            Column {
                Surface(shape = RoundedCornerShape(14.dp), color = MaterialTheme.colorScheme.tertiary.copy(alpha = .10f), border = BorderStroke(1.dp, MaterialTheme.colorScheme.tertiary.copy(alpha = .22f))) {
