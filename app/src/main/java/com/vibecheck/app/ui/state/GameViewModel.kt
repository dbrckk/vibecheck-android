...
class GameViewModel(
    ...
    fun selectMode(mode: GameMode) {
        ...
        if (mode == GameMode.RED_GREEN) savedStateHandle[KEY_SESSION_SEED] = System.currentTimeMillis()
    }
    ...
    fun quickStartMode(mode: GameMode) {
        ...
        } else savedStateHandle[KEY_SCREEN] = AppScreen.PLAYERS.name
    }
    ...
    fun openSettings() { savedStateHandle[KEY_SCREEN] = AppScreen.SETTINGS.name }
    fun openLeaderboard() { savedStateHandle[KEY_SCREEN] = AppScreen.LEADERBOARD.name }
    ...
    fun addPlayer(player: String) { savedStateHandle[KEY_PLAYERS] = ArrayList(players.value + player) }
    fun removePlayer(player: String) { savedStateHandle[KEY_PLAYERS] = ArrayList(players.value.filterNot { it == player }) }
    ...
    fun startGame() {
        ...
        clearSoloState()
        resetKnowMeState()
        if (challengeTarget.value == NO_CHALLENGE) savedStateHandle[KEY_SESSION_SEED] = System.currentTimeMillis()
        savedStateHandle[KEY_SESSION_INSTANCE_ID] = System.nanoTime()
        savedStateHandle[KEY_SCREEN] = AppScreen.GAME.name
    }
    ...
    fun answer(questionId: String, answer: String, isLastQuestion: Boolean) {
        ...
        savedStateHandle[KEY_VOTES] = ArrayList(SessionCodec.encodeVotes(currentVotes + Vote(questionId, answer)))
        if (isLastQuestion) savedStateHandle[KEY_SCREEN] = AppScreen.RESULT.name
        else savedStateHandle[KEY_QUESTION_INDEX] = questionIndex.value + 1
    }

    fun setKnowMeSecret(answer: String) {
        if (modeName.value != GameMode.KNOWS_ME.name || answer.isBlank() || knowSecretAnswer.value.isNotBlank()) return
    ...
    fun submitKnowMeGuess(answer: String, isLastQuestion: Boolean) {
        if (modeName.value != GameMode.KNOWS_ME.name || screenName.value == AppScreen.RESULT.name) return
        ...
        if (answer == secret) scores[index] = scores[index] + 1
        savedStateHandle[KEY_KNOW_SCORES] = scores
        if (index < guessers.lastIndex) {
            savedStateHandle[KEY_KNOW_GUESSER_INDEX] = index + 1
            savedStateHandle[KEY_KNOW_HANDOFF] = true
        } else if (isLastQuestion) savedStateHandle[KEY_SCREEN] = AppScreen.RESULT.name
        else {
    ...
    fun confirmKnowMeHandoff() {
        if (modeName.value == GameMode.KNOWS_ME.name) savedStateHandle[KEY_KNOW_HANDOFF] = false
    }
    ...
    private fun ensureKnowScoreSlots() {
        ...
        if (knowScores.value.size != needed) savedStateHandle[KEY_KNOW_SCORES] = ArrayList(List(needed) { 0 })
    }
    ...
    private fun initialScreenFor(mode: GameMode): AppScreen = if (mode == GameMode.RED_GREEN) AppScreen.GAME else AppScreen.PLAYERS
